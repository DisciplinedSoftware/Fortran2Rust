...
# Hardcoded C fallbacks for routines whose .f source uses Fortran 90+ features
# that f2c (Fortran 77 only) cannot handle. Keyed by lowercase file stem.
_F77_FALLBACKS: dict[str, str] = {
    "xerbla": """\
/* xerbla.c — f2c-compatible fallback (generated by Fortran2Rust) */
/* Original Fortran uses LEN_TRIM (F90), which f2c cannot handle.  */
#include <stdio.h>
#include <stdlib.h>
#include "f2c.h"
...
int xerbla_(char *srname, integer *info, ftnlen srname_len)
{
    fprintf(stderr,
        ...
        (int)srname_len, srname, (int)*info);
    return 0;
}
""",
    "xerbla_array": """\
/* xerbla_array.c — f2c-compatible fallback (generated by Fortran2Rust) */
#include <stdio.h>
#include <stdlib.h>
#include "f2c.h"

int xerbla_array_(char *srname_array, integer *srname_len,
                  integer *info, ftnlen srname_array_len)
{
    (void)srname_array_len;
    fprintf(stderr,
        " ** On entry to %.*s parameter number %d had an illegal value\\n",
        (int)*srname_len, srname_array, (int)*info);
    return 0;
}
""",
}
...
def run_f2c(source_dir: Path, fortran_files: list[Path], output_dir: Path, status_fn=None) -> dict:
    ...
    for i, f in enumerate(fortran_files):
        ...
        if c_path.exists():
            ...
            stem = dest_f.stem.lower()
            if stem in _F77_FALLBACKS:
                c_path.write_text(_F77_FALLBACKS[stem])
                c_files.append(str(c_path))
                if status_fn:
                    status_fn(f"f2c: using built-in C fallback for {dest_f.name}")
            else:
